...
| **Build script contract** | `scripts/guard-build-script-contract.mjs` | `scripts.build` calling OpenNext; missing or wrong `scripts["cf:build"]` | `build` → `next build`; `cf:build` → `opennextjs-cloudflare build`; deploy/preview use `cf:build` |
...
| `package-correct-build.json` | Correct `build` / `cf:build` split (should pass) |
