...
| Env placeholder guard fails | Missing `.env.example` or real-looking values in `.env.example` | Add file with empty values (`KEY=`) or short documented placeholders | `scripts/guards/env-placeholder.ts` |
...
| Cloudflare deploy log says Payload migrated, but live D1 is still missing tables | `deploy:database` ran `payload migrate` without `PAYLOAD_MIGRATE_REMOTE=true`, so migrations updated Wrangler's local D1 simulation | Update deploy scripts from starter; in existing child projects review `.bemoat/package-sync-proposal.md` or run sync with `--apply-build-contract`, then redeploy | [cloudflare-environments.md](../cloudflare-environments.md), `scripts/guards/build-script-contract.ts` |
...
| Package manager drift guard | `package-lock.json` / `yarn.lock` tracked, or npm in harness workflow | Remove alien lockfiles; use pnpm only; keep `engines.pnpm` | `scripts/guards/package-manager.ts` |
| Toolchain contract guard fails | TypeScript, Node, lockfile, or strict harness compiler contract drift | Restore the registered toolchain contract and managed harness roots | `scripts/guards/toolchain-contract.ts` |
...
| Frontend SEO guard fails | Missing `metadata` in `src/app/(frontend)/layout.tsx` | Export `metadata` with `title` and `description` | `scripts/guards/frontend-seo.ts` |
