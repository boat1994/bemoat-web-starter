...
/**
 * Synthetic legacy bodies for retirement fail-closed probes only.
 */
...
const missingCanonicalEvidence = /STATE_CONFLICT: live REVIEW_VERDICT is missing canonical PR\/base\/head evidence/

describe('retired legacy REVIEW_VERDICT binding', () => {
  it('fails closed on legacy Task/PR/Base/Head transport without canonical PR/base/head line', () => {
    const harness = createGhHarness({
      prState: 'MERGED',
      containedHeads: [reviewedHead],
      comments: [{
    ...
    const result = runReconcile(harness)
    expect(result.status).toBe(1)
    expect(result.stderr).toMatch(missingCanonicalEvidence)
    expect(readMetrics(harness).issueEdits).toBe(0)
    expect(readHarnessState(harness).latest_review_verdict_comment_id).toBeUndefined()
  })

  it('fails closed on byte-faithful retired legacy comment 5163387315 body', () => {
...
`,
  ...
  ])('rejects retired legacy %s without writing managed state', (_label, overrides, expected) => {
    const harness = createGhHarness({
      prState: 'MERGED',
      containedHeads: [reviewedHead],
      ...overrides,
  ...
  it('keeps canonical verdict authoritative when retired legacy different-Issue comment is non-competing', () => {
    const harness = createGhHarness({
      prState: 'MERGED',
      containedHeads: [reviewedHead],
      comments: [
...
**Head:** \`${reviewedHead}\`
`],
  ])('rejects multiline/case/boundary retired legacy form: %s', (_label, body) => {
    const harness = createGhHarness({
      prState: 'MERGED',
      containedHeads: [reviewedHead],
      comments: [{
