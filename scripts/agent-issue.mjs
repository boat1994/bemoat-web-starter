...
import {
  findHistoricalCommentByDatabaseId,
...
function verifyReviewThreeCorrectionAuthorization({ issueBody, contract, comments }) {
  ...
  const latestHandoff = selectCurrentRoleComment(comments, 'HANDOFF')
  const historicalHandoff = findHistoricalCommentByDatabaseId(
    comments,
    authorization.handoff_comment_id,
    'HANDOFF',
  )
  if (!historicalHandoff.ok && historicalHandoff.errors.some((error) => /duplicated/.test(error))) {
    return { ok: false, errors: historicalHandoff.errors }
  }
  if (!historicalHandoff.ok || String(latestHandoff?.comment?.id) !== String(authorization.handoff_comment_id)) {
    return { ok: false, errors: ['STATE CONFLICT: Founder correction authorization is not bound to its exact active HANDOFF'] }
  }
  const verification = verifyHistoricalReview3Authority({
    authorization,
    activePr: state.active_pr,
    handoff: historicalHandoff.comment,
    contract,
  })
  return verification.ok
    ? { ok: true, errors: [], reviewThree: true, proof: verification.proof }
    : { ok: false, errors: verification.errors, reviewThree: true }
}
...
function runCorrectionPhasePreflight({
  ...
  const latestVerdict = selectCurrentRoleComment(commentResult.comments, 'REVIEW_VERDICT')
