...
import { recoverCurrentAuthority as recoverCurrentAuthorityGrant } from './agent-issue/current-post-budget-authority.mjs'
...
function fetchPullReviewCommentById(cwd, commentId, env = process.env) {
  const defaultRepo = getDefaultRepo(cwd)
  if (!defaultRepo || !/^[1-9]\d*$/.test(String(commentId))) {
