...
type PrIdentity = { owner: string; repo: string; number: string; key: string }
type ParseResult = { ok: true; identity: PrIdentity } | { ok: false; reason: string }
type ParseFailure = { ok: false; reason: string }
type CanonicalIdentity = PrIdentity | { none: true }
type VerdictIdentityResult = { ok: true; identity: CanonicalIdentity } | { ok: false; errors: string[] }
type ParsedTargetResult =
  | { ok: true; none: true; identity: { none: true } }
...
function isParseFailure(value: ParseResult): value is ParseFailure {
  return value.ok === false
}
type Finding = { id?: unknown; source_thread?: unknown }
type VerdictContract = { findings?: Finding[] }

function asciiCaseFold(value: unknown): string {
  return String(value).toLowerCase()
}

function foldedPrIdentityKey(owner: string, repo: string, number: string): string {
 ...
 */
export function parseCompleteGitHubPullUrl(raw: unknown): ParseResult {
...
function isSourceThreadDiscussionPointer(candidate: unknown, canonicalIdentity: CanonicalIdentity | null, knownSourceThreads: Set<string> | null = null): boolean {
  ...
  if (isNoneIdentity(canonicalIdentity)) return Boolean(knownSourceThreads?.has(candidate))
...
function isPlausiblePullIdentityCandidate(candidate: unknown): boolean {
...
export function extractVerdictPrBaseAndHead(verdictBody: string): { base: string | null; head: string | null } {
...
function extractCanonicalPrTargetLines(verdictBody: string): string[] {
  return [...verdictBody.matchAll(CANONICAL_PR_TARGET_LINE_RE)].map((match) => match[1] ?? '')
}

function collectMalformedPrIdentityCandidates(verdictBody: string, canonicalIdentity: CanonicalIdentity | null = null, knownSourceThreads: Set<string> | null = null): { malformedCandidates: Array<{ candidate: string; reason: string; source: 'url' }> } {
  const malformedCandidates: Array<{ candidate: string; reason: string; source: 'url' }> = []
  const seenMalformed = new Set()
  const consider = (rawCandidate: string): void => {
...
function parseCanonicalPrTargetLine(line: string, defaultRepo: string): ParsedTargetResult {
...
function validateFindingSourceThreads(canonicalIdentity: CanonicalIdentity, contract: VerdictContract | null): { ok: boolean; errors: string[] } {
  const errors: string[] = []
  if (!contract?.findings?.length) return { ok: true, errors }
...
export function resolveCanonicalVerdictPrIdentity(verdictBody: string, defaultRepo: string | null | undefined, mode: 'implementation_pr' | 'planning_no_pr' = 'implementation_pr', knownSourceThreads: Set<string> | null = null, contract: VerdictContract | null = null): VerdictIdentityResult {
  ...
  if (parsedTarget.ok === false) return { ok: false, errors: parsedTarget.errors }
...
export function collectKnownSourceThreads(contract: VerdictContract | null): Set<string> {
