...
function readInputProperty(value: unknown, key: string): unknown { if (value === null || value === undefined) throw new TypeError(`Cannot destructure property '${key}' of ${value} as it is ${value}`); return Reflect.get(Object(value), key) }
function isStringArray(value: unknown): value is string[] { return Array.isArray(value) && value.every((entry) => typeof entry === 'string') }
function filterComments(comments: unknown, callback: (entry: unknown) => boolean): unknown[] { const filter = readProperty(comments, 'filter'); if (typeof filter !== 'function') throw new TypeError('comments.filter is not a function'); const filtered: unknown = Reflect.apply(filter, comments, [callback]); if (!Array.isArray(filtered)) throw new TypeError('comments.filter did not return an array'); return filtered }
