...
function filterComments(comments: unknown, callback: (entry: unknown) => boolean): unknown[] { const invoke = (container: { filter: (predicate: (entry: unknown) => boolean) => unknown[] }): unknown[] => container.filter(callback); return Reflect.apply(invoke, undefined, [comments]) }
