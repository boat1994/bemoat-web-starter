...
function readBulletValue(body, label) {
...
function parseIssueOrPr(value) {
  const match = String(value ?? '').match(/#?([1-9]\d*)/)
  return match ? match[1] : null
}

function parseBaseBinding(value) {
...
function extractBacktickListAfter(body, label) {
  const escaped = label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
...
function extractSingleBacktickAfter(body, label) {
 ...
 * @returns {object}
 */
export function parseFounderAdoptFindingAuthorization(body = '') {
...
export function assertFounderAdoptFindingAuthorization({
  ...
  const bodyHash = hashExactBody(comment.body)
...
function assertNonSupersession({ comments, targetComment, authorization }) {
...
function assertNoCompetingAuthorizations({ comments, targetComment, options }) {
  const targetId = String(targetComment.id)
  const competitors = []
  for (const comment of comments) {
    if (String(comment?.id) === targetId) continue
