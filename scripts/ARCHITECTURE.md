...
- Adapter isolation, ensuring adapters (like `scripts/adapters/command-runner.ts`) do not import from the repository and are only used by allowlisted callers.
- A complete root-script migration map: every `scripts/*.ts`, `scripts/*.mjs`, and `scripts/*.sh` file has exactly one mapping with facade disposition, internal destination vocabulary, owning slice (1–7), and migration status.
...
`scripts/guard-harness-contract.ts` is the stable public facade and direct CLI composition root for harness-contract checks.

Owned modules under `scripts/harness-contract/` may import Node builtins and approved intra-directory modules only:

| Module | Responsibility |
| --- | --- |
| `scripts/harness-contract/child-script-policy.ts` | Child-facing `bemoat:*` policy scanning and formatting |
| `scripts/harness-contract/runtime-import-parser.ts` | Static/dynamic runtime import specifier parsing |
| `scripts/harness-contract/managed-runtime-closure.ts` | Managed runtime delivery closure scan/assert |
| `scripts/harness-contract/manifest.ts` | Managed-paths manifest loading |
