...
if [ -z "$branch" ]; then
  ...
  echo "  git switch -c chore/67-git-flow-branch-guardrails develop" >&2
  exit 1
fi
...
case "$branch" in
  main)
    ...
    echo "Create a topic branch from develop, for example:" >&2
    echo "  git switch -c chore/67-git-flow-branch-guardrails develop" >&2
    exit 1
    ;;
  develop)
    if [ "${ALLOW_INTEGRATION_BRANCH:-}" = "1" ]; then
      echo "Branch safety: integration maintenance bypass enabled for develop."
      exit 0
    fi

    echo "Branch safety failed: develop is an integration branch, not a routine implementation branch." >&2
