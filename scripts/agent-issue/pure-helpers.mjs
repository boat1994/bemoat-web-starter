...
export function buildSuggestedBranchName(issueNumber, issueTitle) {
...
export function extractBacktickPath(text) {
  const match = text.match(/`([^`]+)`/)
  return match ? match[1].trim() : text.trim()
}

export function isMeaningfulIssueRef(value) {
...
export function parseIssueFormSection(source, headingPrefix) {
...
export function assignDeclarationValue(declarations, key, value, options = {}) {
  if (!value) return
  const trimmed = value.trim()
