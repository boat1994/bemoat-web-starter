...
export function readLocalGitEvidence({ cwd, run }: { cwd: string; run: ContextCommandRunner }): LocalGitEvidence {
  ...
  const remoteHead = upstream ? output(run('git', ['rev-parse', `refs/remotes/${upstream}`], { cwd })) : null
  const clean = status === ''
  ...
  const pushed = Boolean(head && upstream && upstreamMatchesBranch && remoteHead && head === remoteHead)
  ...
  if (!pushed) reasons.push('LOCAL_STATE_NOT_DURABLE: current HEAD is not proven pushed to its upstream')
