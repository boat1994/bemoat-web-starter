  ...
  return {
    'bemoat:handoff': contract({
      ...
      required_evidence: [
        ...
        'No applicable active PR for read_only mode.',
        'Authoritative changed-file evidence determining the implementation validation tier or proving an empty diff for read_only mode.',
      ...
      last_validation_before_mutation: 'Run the repository-required validation after the clean exact-head preflight (read_only requires empty changed-file evidence and uses `pnpm run bemoat:guard:safety`), add runtime-generated PASS proof bound to the tier, canonical command, and exact HEAD, then re-read the complete HANDOFF schema, repository, Issue, protected-base, branch/head/upstream, changed-file scope, and applicable PR bindings immediately after validation and before posting.',
