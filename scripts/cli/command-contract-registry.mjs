...
const commands = {
  ...
  'bemoat:mission-control:recover-review': contract({
    ...
    required_inputs: [
      ...
      stdinInput('review_body', 'Recovery review body when --body-file is not used.'),
    ],
    optional_flags: [
      flag('repository', '--repo <owner/repository>', 'repository', 'Exact starter repository.'),
      flag('expected_pr', '--expected-pr <number>', 'positive_integer', 'Exact incident Pull Request number.'),
      flag('expected_base', '--expected-base <branch>', 'string', 'Exact protected base branch.'),
      flag('expected_state', '--expected-state <state>', 'enum', 'Expected incident managed state.', ['AWAITING_REVIEW_2']),
      flag('expected_head', '--expected-head <full-sha>', 'full_sha', 'Exact incident PR head.'),
      flag('expected_review_cycle', '--expected-review-cycle <number>', 'positive_integer', 'Expected managed review cycle.'),
      flag('expected_full_review_count', '--expected-full-review-count <number>', 'positive_integer', 'Expected full-review count.'),
      flag('review_type', '--review-type <type>', 'enum', 'Exact recovery review type.', ['delta']),
      flag('issue_source_comment', '--issue-source-comment <id>', 'positive_integer', 'Pinned Task Issue source comment.'),
      flag('pr_source_comment', '--pr-source-comment <id>', 'positive_integer', 'Pinned PR source comment.'),
      flag('original_review_comment', '--original-review-comment <id>', 'positive_integer', 'Pinned original review comment.'),
      flag('correction_result_comment', '--correction-result-comment <id>', 'positive_integer', 'Pinned correction RESULT comment.'),
      flag('body_file', '--body-file <path>', 'path', 'Read recovery review body from this file.'),
    ],
    required_evidence: [
    ...
    examples: [
      {
        ...
        argv: ['274', '--repo', 'boat1994/bemoat-web-starter', '--expected-pr', '275', '--review-type', 'delta'],
      },
    ],
  ...
  'bemoat:mission-control:review': contract({
    ...
    required_inputs: [
      positional('issue_number', '<issue-number>', 'positive_integer', 'Managed Task Issue number.'),
      stdinInput('review_body', 'REVIEW_VERDICT body when --body-file is not used.'),
    ...
    optional_flags: [
      flag('repository', '--repo <owner/repository>', 'repository', 'Repository containing the Task Issue.'),
      flag('body_file', '--body-file <path>', 'path', 'Read the review body from this file instead of stdin.'),
    ],
