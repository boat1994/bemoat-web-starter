...
export function missionControlRecoveryCommands(dependencies: CommandMetadataDependencies) {
  ...
  'bemoat:mission-control:recover-review-eligibility': contract({
    ...
    required_inputs: [
      ...
      flag('protected_main_sha', '--protected-main-sha <full-sha>', 'full_sha', 'Exact current protected main commit used for policy provenance.', [], true),
    ...
    stop_conditions: [
      ...
      'No REVIEW_VERDICT comment, review counter increment, comment edit, or adoption/finding action is permitted.',
    ],
    examples: [{
      description: 'Non-mutating exact review-eligibility recovery validation.',
      argv: ['380', '--repo', 'boat1994/bemoat-web-starter', '--expected-pr', '390', '--expected-base', 'main', '--expected-base-sha', '<recorded-pr-base-sha>', '--protected-main-sha', '<protected-main-sha>', '--expected-head', '<current-head>', '--expected-branch', 'fix/380-mission-control-stabilization', '--result-comment', '<result-comment-id>', '--check'],
    }],
