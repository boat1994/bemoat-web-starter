...
export function missionControlRecoveryCommands(dependencies) {
  ...
  'bemoat:mission-control:rebind-review-lineage': contract({
    command: 'bemoat:mission-control:rebind-review-lineage',
    tier: 'A',
    entrypoint: 'scripts/mission-control-rebind-review-lineage.mjs',
    purpose: 'Quarantine only the registered legacy REVIEW_VERDICT lineage and rebind it to canonical PR/base/head transport.',
    operation: 'Verify the pinned #259/#260 tuple and Founder authorization, post the canonical REVIEW_VERDICT, demote the source comment, and CAS-update only lineage identity.',
    accepted_pre_states: ['ELIGIBLE_FOR_FOUNDER_REVIEW'],
    required_inputs: [
      positional('issue_number', '<issue-number>', 'positive_integer', 'Registered Task Issue number.'),
      flag('repository', '--repo <owner/repository>', 'repository', 'Exact starter repository.', [], true),
      flag('expected_pr', '--expected-pr <number>', 'positive_integer', 'Exact registered Pull Request number.', [], true),
      flag('expected_base', '--expected-base <branch>', 'string', 'Exact protected base branch.', [], true),
      flag('expected_state', '--expected-state <state>', 'enum', 'Expected managed state.', ['ELIGIBLE_FOR_FOUNDER_REVIEW'], true),
      flag('expected_head', '--expected-head <full-sha>', 'full_sha', 'Exact registered PR head.', [], true),
      flag('expected_review_cycle', '--expected-review-cycle <number>', 'positive_integer', 'Expected managed review cycle; must remain unchanged.', [], true),
      flag('expected_full_review_count', '--expected-full-review-count <number>', 'positive_integer', 'Expected full-review count; must remain unchanged.', [], true),
      flag('source_comment', '--source-comment <id>', 'positive_integer', 'Registered legacy REVIEW_VERDICT source comment.', [], true),
      flag('authorization_comment', '--authorization-comment <id>', 'positive_integer', 'Immutable Founder lineage-rebind authorization comment.', [], true),
      flag('body_file', '--body-file <path>', 'path', 'Canonical REVIEW_VERDICT body that preserves existing provenance.', [], true),
    ],
    trusted_derived_values: [
      'registered #259/#260/source-comment/head tuple',
      'Founder authorization identity and body',
      'legacy source-comment binding',
      'canonical REVIEW_VERDICT comment id',
      'lease/CAS holder identity',
      'unchanged review counters and ELIGIBLE_FOR_FOUNDER_REVIEW state',
    ],
    required_evidence: [
    ...
    reads: [
      'Task Issue managed state',
      'live PR number/base/head',
      'legacy source REVIEW_VERDICT comment',
      'Founder authorization comment',
      'Issue comments for competing verdicts',
    ],
    writes: [
      'one canonical REVIEW_VERDICT comment',
      'in-body demotion of the registered source comment',
      'leased/CAS update of latest_review_verdict_comment_id and latest_transition_identity only',
    ],
    success_classifications: ['SUCCESS', 'NO_OP_IDENTICAL_RETRY'],
    retry_contract: {
      identical_retry: 'conditional',
      classification: 'NO_OP_IDENTICAL_RETRY',
      condition: 'A retry is identical only when the same canonical REVIEW_VERDICT, demoted source comment, and lineage projection are already durable.',
    },
    next_action_rules: [
      {
        classification: 'SUCCESS',
        next_action: nextAction('COMPLETE', null, 'The registered lineage-transport rebind is verified.'),
      },
      {
        classification: 'NO_OP_IDENTICAL_RETRY',
        next_action: nextAction('COMPLETE', null, 'The identical lineage-transport rebind is already durable.'),
      },
    ],
    stop_conditions: [
      'Stop when the invocation tuple does not match the registered #259/#260 lineage exactly.',
      'Stop when Founder authorization is missing, unbound, or mismatched.',
      'Stop on competing undemoted REVIEW_VERDICT evidence.',
      'Stop on partial or ambiguous mutation without inventing a second semantic review.',
      'This command is quarantined and retires after required legacy lineage migrations complete.',
    ],
    examples: [
      {
        description: 'Run the quarantined #259/#260 lineage-transport rebind.',
        argv: [
          '259',
          '--repo',
          'boat1994/bemoat-web-starter',
          ...
          '--expected-head',
          'b1ce5f58e7ffd0178d955ef7e93395209a7c4d28',
    ...
    last_validation_before_mutation: 'Verify the complete registered tuple, Founder authorization on Issue #259 including exact replacement/source SHA-256 and Findings provenance, and absence of competing undemoted verdicts immediately before comment and CAS writes.',
